    ...
    if (comic_content === null) {
        return (
            <div>
                ...
                <input type="file" ref={fileInputRef} name="comic_content[]" multiple  className="mb-10 text-white hidden" id="comics-create_field"  onChange={(e) => setComicContent(addFiles(e.target.files, comic_content))} /><br />
            </div>
        )
        ...
        return (
            <div>
                    ...
                    {comic_content.map((file, index) => {
                        ...
                        return (
                          <div className="bg-gray-200 w-[600px] m-auto px-20 pb-10 pt-20 flex my-[32px] relative">
                            <img src="./images/batu_icon.svg" alt="" className="w-20 absolute top-6 right-6"/>
                ...
                <input type="file" name="comic_content[]" multiple  className="mb-10 text-white hidden" id="comics-create_field"  onChange={(e) => setComicContent(addFiles(e.target.files, comic_content))} /><br />
            </div>
        )
    } else if (comic_content.length > 1) {
        return (
                        ...
                        return (
                            <div className="">
                                <div className="bg-gray-200 w-[200px] m-auto px-4 pb-2 pt-4 flex my-[32px] relative">
                                    <img src="./images/batu_icon.svg" alt="" className="w-10 absolute top-2 right-2"/>
                                    <img src={fileURL} alt={`image-${index}`} className="m-auto border-2 border-black"/>
                                </div> 
                            </div> 
                        );
